...
## Sites that NEED agent-browser (web_fetch returns empty/broken):

- **docs.stripe.com** — JS-rendered SPA, web_fetch gets nav links only
- **vercel.com/guides** and **vercel.com/templates** — many pages 404 or return minimal content via web_fetch
- **www.peopledatalabs.com/pricing** — JS-rendered SPA, web_fetch returns page title only. `docs.peopledatalabs.com` works fine (server-rendered). `support.peopledatalabs.com` returns 403 (Cloudflare challenge).
- Any SPA/React/Next.js docs site — try web_fetch first, fall back to agent-browser if content is suspiciously short
